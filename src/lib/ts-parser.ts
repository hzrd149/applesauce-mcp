...
function extractMethodInfo(
  ...
  const lineNumber =
    sourceFileObj.getLineAndCharacterOfPosition(node.getStart()).line + 1;
...
function hasExportModifier(node: ts.Node): boolean {
  ...
  return modifiers.some((mod: ts.Modifier) =>
...
export async function getAllTypeScriptFiles(dir: string): Promise<string[]> {
  ...
  for await (const entry of Deno.readDir(dir)) {
    ...
    if (entry.isDirectory) {
      ...
      if (
        ["node_modules", "dist", "build", "__tests__", ".git"].includes(
          entry.name,
        )
      ) {
        continue;
...
export function mapSourceToExportPath(
  ...
  for (const [exportPath, exportValue] of Object.entries(exports)) {
    if (typeof exportValue === "object" && exportValue !== null) {
      ...
      const importPath = exportObj.import || exportObj.types ||
        exportObj.require;
      ...
      if (importPath) {
        ...
        if (srcPath.includes("*")) {
          ...
          // Exact match
          if (
            normalizedSource.endsWith(srcPath) ||
            normalizedSource.includes(srcPath.replace(/\/index\.ts$/, "/"))
          ) {
            const specificity = srcPath.length;
            if (!bestMatch || specificity > bestMatch.specificity) {
              const cleanExportPath = exportPath === "."
                ? ""
                : exportPath.replace(/^\.\//, "");
              bestMatch = { exportPath: cleanExportPath, specificity };
            }
